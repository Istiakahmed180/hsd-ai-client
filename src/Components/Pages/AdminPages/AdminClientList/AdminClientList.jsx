...
const AdminClientList = () => {
  ...
  const handleClientDelete = async (id) => {
    try {
      const response = await axios.delete(
        `http://ec2-13-229-77-43.ap-southeast-1.compute.amazonaws.com/api/invest/delete-client/${id}`
      );
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("http://ec2-13-229-77-43.ap-southeast-1.compute.amazonaws.com/api/invest/all-client")
      .then((res) => {
        setTimeout(() => {
  ...
  return (
    <div className="container mx-auto px-6 py-8 min-h-screen">
      <div className="bg-blue-900 text-white px-6 py-4 rounded-tl-2xl rounded-tr-2xl">
        <h1 className="text-2xl font-medium">Client List</h1>
      </div>
      <div className="mt-6">
                ...
                return (
                  <tr key={client?._id}>
                    ...
                    <td className="py-4 pl-12 border-b text-white">
                      {client?.name ? client?.name : "No Information"}
                    </td>
                    ...
                    <td className="py-4 pl-12 border-b text-orange-600 font-semibold">
                      <div className="flex items-center">
                        <FaDollarSign />
                        <span>
                          {client?.amount ? client?.amount : "No Information"}
                        </span>
                      </div>
                    </td>
                    <td className="py-4 pl-12 border-b text-white">
          ...
          <div className=" bg-[#05003B] w-1/2 rounded-2xl shadow-lg z-50">
            <div className="py-6 text-left px-6">
              ...
              <div className="flex justify-between space-x-8 my-5">
                <Image
                  src={checkClient?.image}
                  width={250}
                  height={250}
                  alt="Client Image"
                  className="rounded-3xl"
                ></Image>
                <p className="text-justify">{checkClient?.bio}</p>
              </div>
              ...
              <div className="px-20">
                <div className="flex justify-between   text-lg my-5">
                  ...
                  <div className="flex items-center text-yellow-400 font-semibold">
                    <FaDollarSign />
                    <span>
                      {checkClient?.amount ? checkClient?.amount : "0"}
                    </span>
                  </div>
                </div>
                <div className="flex justify-between text-lg my-5">
